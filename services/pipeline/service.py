...
from services.storage.postgres_async_staging_storage import PostgresAsyncStagingStorageService
from services.pipeline.normalizer import normalize_record
from services.pipeline.stream_reader import iter_batches


class IngestService:
	def __init__(self, staging_storage: PostgresAsyncStagingStorageService | None = None) -> None:
		self.staging_storage = staging_storage or PostgresAsyncStagingStorageService()
	...
	async def ingest(
		...
		try:
			for batch in iter_batches(input_path, batch_size=batch_size, limit=limit):
				...
				try:
					inserted = await self.staging_storage.bulk_insert(normalized_batch)
